...
tfr_izq.data = tfr_izq.data.astype(np.float32)
tfr_der.data = tfr_der.data.astype(np.float32)
...
for banda in [parameters["banda_mu"], parameters["banda_beta"], parameters["banda_completa"]]:
    ...
    plt.tight_layout()
    fig.canvas.manager.set_window_title(f"Análisis de tiempo frecuencia - Banda {banda[0]}-{banda[1]}Hz - {tipo_sesion} - Grand Average")
    if save:
        plt.savefig(os.path.join(root_path, f"timefreq_ga_{tipo_sesion}_{banda}.png"), dpi=350, bbox_inches='tight')
    if show:
        plt.show()
    #elimino la figura
    plt.close(fig)