...
sujetos = [1,4,8] #parameters["sujeto"]
...
epocas = mne.Epochs(data, event_id=["IZQUIERDA", "DERECHA"],
                                 tmin=tmin-0.1, tmax=tmax+0.1,
                                 baseline=None, preload=True,reject={"eeg":amplitude_rejection})

raw_eventos = mne.events_from_annotations(data, event_id=event_ids)
...
chizq_i = [data.ch_names.index(chs_names[0])] #índice canal izquierdo
chder_i = [data.ch_names.index(chs_names[1])] #índice canal derecho
cluster_izq = [data.ch_names.index(ch) for ch in parameters["cluster_electrodos_izq"]]
cluster_der = [data.ch_names.index(ch) for ch in parameters["cluster_electrodos_der"]]
