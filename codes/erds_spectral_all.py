...
from codes.utils import concatenateSubjectsEEG, getIntervalos
...
eeg_concatenados = concatenateSubjectsEEG(sujetos, sesion, apply_ica=True).drop_channels(channels_to_drop, "ignore")
...
fig.canvas.manager.set_window_title(f"IZQUIERDA {tipo_sesion} - Grand Average")
plt.suptitle(f"IZQUIERDA {tipo_sesion} - Grand Average", fontsize=title_fontsize)
if save:
    plt.savefig(os.path.join(root_path, f"spectral_izq_ga_{tipo_sesion}.png"), dpi=350)
if show:
    plt.show()
plt.close(fig)
...
axes[3].legend(loc="upper right", fontsize=label_fs)
axes[3].set_xlabel("Frecuencia (Hz)", fontsize=label_fs)
plt.suptitle(f"DERECHA {tipo_sesion} - Grand Average", fontsize=title_fontsize)
fig.canvas.manager.set_window_title(f"DERECHA {tipo_sesion} - Grand Average")
if save:
    plt.savefig(os.path.join(root_path, f"spectral_der_ga_{tipo_sesion}.png"), dpi=350, bbox_inches='tight')
if show:
    plt.show()
plt.close(fig)
